nies_invasive_species_of_japan_2002_Download_and_Depurate <- function(){
  ...
  resultados5 <- list(resultados4)
  resultados6 <- data.frame(resultados5)
  resultados6$Scientific_name <- resultados4

  #No duplicados
  source(file.path("R", "noduplicates.r"))
  dat_noduplicados <- noduplicates(resultados6, "Scientific_name")

  dat_noduplicados$Habitat_Database <- "Freshwater"
  dat_noduplicados$Invaded_Country <- "Japan"
  dat_noduplicados <- dat_noduplicados[-1,]

  write.xlsx(dat_noduplicados, "Inputfiles/originaldatabase_nies_invasive_species_of_japan_2002.xlsx")
  write.xlsx(dat_noduplicados, "Inputfiles/freshwatersubset_nies_invasive_species_of_japan_2002.xlsx")
}
