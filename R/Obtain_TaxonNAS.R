Obtain_TaxonNAS <- function() {
  ...
  taxonomy_withNCBI <- read.xlsx("OutputFiles/Intermediate/taxonomy_withGBIF.xlsx")
  ...
  dataset_na <- taxonomy_withNCBI %>%
    filter(if_any(all_of(tax_cols), is.na)) %>%
    select(ID_GBIF, AcceptedNameGBIF, everything())

  species_to_query <- dataset_na$AcceptedNameGBIF
  ids_to_query <- dataset_na$ID_GBIF

  ncbi_query <- function(sp, id) {
    result <- tryCatch(
      tax_name(
        sci = sp,
        get = c("kingdom", "order", "family", "class", "phylum"),
        db = "ncbi"
      ),
      error = function(e) {
        return(data.frame(
          kingdom = NA, order = NA, family = NA,
          class = NA, phylum = NA
        ))
      }
    )
  ...
  progressr::with_progress({
    ...
    results <- future_lapply(seq_along(species_to_query), function(i) {
      sp <- species_to_query[i]
      id <- ids_to_query[i]

      res <- ncbi_query(sp, id)
  ...
  taxonomy_withNCBI <- taxonomy_withNCBI %>%
    left_join(
      taxonomy_df %>% select(ID_GBIF, Kingdom, Order, Family, Class, Phylum),
      by = "ID_GBIF",
      suffix = c("", ".ncbi")
    )
  for (col in tax_cols) {
    taxonomy_withNCBI[[col]] <- ifelse(
      is.na(taxonomy_withNCBI[[col]]),
      taxonomy_withNCBI[[paste0(col, ".ncbi")]],
      taxonomy_withNCBI[[col]]
    )
  }
  taxonomy_withNCBI <- taxonomy_withNCBI %>%
    select(-ends_with(".ncbi"))
  ...
  mutate(
    Kingdom = case_when(
      ...
      Kingdom == "Bacteria" ~ "Bacillati",

      is.na(Kingdom) & Group %in%
        c("Amphibian","Arachnid","Bird","Crustacean","Fish",
          "Insect","Mammal","Mollusc","Other invertebrate","Reptile") ~ "Animalia",

      is.na(Kingdom) & Group == "Fungus" ~ "Fungi",
      is.na(Kingdom) & Group == "SAR" ~ "Chromista",

      Phylum == "Rhodophyta" ~ "Plantae",
  ...
  mutate(
    Phylum = case_when(
      Class == "Magnoliopsida" ~ "Tracheophyta",

      Phylum %in% c("Angiosperms","Magnoliophyta","Spermatophyta",
                    "Coniferophyta","Pteridophyta","Streptophyta") ~ "Tracheophyta",

      Phylum %in% c("Heterokontophyta","Bacillariophyta") ~ "Ochrophyta",
  ...
  mutate(
    Class = case_when(
      Class %in% c("Teleostei","Teleostomi","Osteichthyes") ~ "Actinopterygii",
      Class == "Elasmobranchii" ~ "Chondrichthyes",
      Class == "Dipneusti" ~ "Dipnoi",
      Class == "Angiospermae" ~ "Magnoliopsida",
      Class == "Monocotyledonae" ~ "Liliopsida",
      Class == "Filicopsida" ~ "Polypodiopsida",
      Class %in% c("Coscinodiscophyceae","Fragilariophyceae") ~ "Bacillariophyceae",
      Class == "Secernentea" ~ "Chromadorea",
      Class == "Gasteropoda" ~ "Gastropoda",
  ...
  mutate(
    Order = case_when(
      ...
      Order == "Cyprinodontiformers" ~ "Cyprinodontiformes",
      Order == "Veneroida" ~ "Venerida",
      Order == "Neotaenioglossa" ~ "Littorinimorpha",
      Order == "Basommatophora" ~ "Hygrophila",
      Order == "Pulmonata" ~ "Stylommatophora",
      Order == "Ostreida" ~ "Ostreoida",
      Order == "Arales" ~ "Alismatales",
      Order == "Callitrichales" ~ "Lamiales",
      Order == "Cyperales" ~ "Poales",
      Order == "Euphorbiales" ~ "Malpighiales",
      Order == "Juncales" ~ "Poales",
      Order == "Typhales" ~ "Poales",
