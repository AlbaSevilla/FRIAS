...
GISD_Download_and_Depurate <- function() {
  ############### DESCARGAR ##################
  ...
  destfile <- "InputFiles/Step0_OriginalDatabase_GISD.xlsx"
  ...
  ############ DEPURAR ##############################
  datos <- read.csv("Inputfiles/Step0_OriginalDatabase_GISD.csv", sep=";")
  head(datos)
  names(datos)
  colnames(datos) <- gsub(" ", ".", colnames(datos))
  columnas_quedarse <- c("Species", "Kingdom", "Order", "Family", "System", "EICAT.Category",
                         "Countries.of.impact", "Impact.mechanism", "Impacted.native.species")
  datos <- datos[,columnas_quedarse]
  ...
  GISD_sinduplicados <- noduplicates(GISD_FRESHWATER, column_name_species = "Species")

  write.xlsx(GISD_sinduplicados, "./Inputfiles/Step0_OriginalDatabaseFreshwaterNODUPLICATES_GISD.xlsx")
  cat("Archivo Excel de GISD guardado como 'Step0_OriginalDatabaseFreshwaterNODUPLICATES_GISD.xlsx'\n")
