HORUS_Download_and_Depurate <- function() {
  ...
  data_name_HORUS <- list.files(path = ruta_inputfiles, pattern = "Step0_OriginalDatabase_HORUS.csv", full.names = TRUE)
  dat <- read.csv("Inputfiles/Step0_OriginalDatabase_Horus.csv")


  #OBTENEMOS ESPECIE ÚNICA POR FILA
  source(file.path("R","noduplicates.R"))
  HORUS_sinduplicados <- noduplicates(dat, "scientific_name")
  ...
  write.xlsx(dataset_freshwater, "./Inputfiles/Step0_OriginalDatabaseFreshwaterNODUPLICATES_HORUS.xlsx")
  cat("Archivo descargado correctamente: /Inputfiles/Step0_OriginalDatabaseFreshwaterNODUPLICATES_HORUS.xlsx")
}
