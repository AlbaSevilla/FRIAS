GapsPerColumn <- function(){
  contar_gaps_df <- function(df, version_name) {
    ...
    data.frame(
      ...
      version = version_name  # "Before" o "After"
    )
  }
  ...
  gaps_combined <- gaps_combined %>%
    arrange(desc(Before)) %>%
    mutate(columna = factor(columna, levels = columna))
  ...
  gaps_long$version <- factor(gaps_long$version, levels = c("Before", "After"))

  # --- Figure 1
  ggplot(gaps_long, aes(x = columna, y = gaps, fill = version)) +
  ...
  ggsave("Figures/GapsPerColumn_superposed.png", plot = p1, width = 10, height = 6, dpi = 300)
  ...
  ggsave("Figures/GapsPerColumn_dodged.png", plot = p2, width = 10, height = 6, dpi = 300)
}
