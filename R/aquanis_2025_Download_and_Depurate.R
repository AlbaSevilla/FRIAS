aquanis_2025_Download_and_Depurate <- function(){
  ...
  extract_information <- function(url, species, idx, total) {
    ...
    tryCatch({
      ...
      clean_native_origin <- function(text) {
        ...
        text <- stringr::str_replace_all(text, "Not entered", "")

        ocean <- stringr::str_match(text, "Ocean:\\s*(\\w+)")[,2]
        country <- stringr::str_match(text, "Country:\\s*(\\w+)")[,2]

        founded_lmes <- valid_lme[stringr::str_detect(text, stringr::fixed(valid_lme, ignore_case = TRUE))]
      ...
      content <- function(tbl) {
        ...
        all(stringr::str_detect(text, stringr::regex(keywords, ignore_case = TRUE)))
      }

      objective_table <- purrr::keep(res2, content)
