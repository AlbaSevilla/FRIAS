##################################################################################################
############ FRIAS : OBTENCIÓN DE LA BBDD DE GISD DE ESPECIES NO-NATIVAS DE AGUA DULCE ##########
##################################################################################################

Global_Invasive_Species_Database_2005_Download_and_Depurate <- function() {
  ############### DESCARGAR ##################
  ...
  destfile <- "InputFiles/Step0_OriginalDatabase_Global_Invasive_Species_Database_2005.xlsx"
  ...
  ############ DEPURAR ##############################
  datos <- read.csv("Inputfiles/Step0_OriginalDatabase_Global_Invasive_Species_Database_2005.csv", sep=";")
  head(datos)
  names(datos)
  colnames(datos) <- gsub(" ", ".", colnames(datos))
  names(datos)[names(datos) == "Countries.of.impact"] <- "Countries_of_impact"
  head(datos)
  ...
  #SELECCIONAMOS LOS FRESHWATER
  unique(datos$System)
  Global_Invasive_Species_Database_2005_FRESHWATER <- datos %>%
    filter(grepl("freshwater", System, ignore.case = TRUE))

  #OBTENEMOS LA COLUMNA EICAT_by_MECHANISM
  names(Global_Invasive_Species_Database_2005_FRESHWATER)

  Global_Invasive_Species_Database_2005_FRESHWATER <- Global_Invasive_Species_Database_2005_FRESHWATER %>%
  ...
  Global_Invasive_Species_Database_2005_sinduplicados <- noduplicates(Global_Invasive_Species_Database_2005_FRESHWATER, column_name_species = "Species")

  write.xlsx(Global_Invasive_Species_Database_2005_sinduplicados, "./Inputfiles/Step0_OriginalDatabaseFreshwaterNODUPLICATES_Global_Invasive_Species_Database_2005.xlsx")
  cat("Archivo Excel de Global_Invasive_Species_Database_2005 guardado como 'Step0_OriginalDatabaseFreshwaterNODUPLICATES_Global_Invasive_Species_Database_2005.xlsx'\n")
