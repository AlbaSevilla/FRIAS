check_habitat <- function(species, dataset) {
  ...
  for (i in seq_along(species)) {
    ...
    message("Procesando especie ", i, " de ", length(species), ": ", specie)
  ...
  # Eliminar duplicados dentro de cada cadena
  dataset$Habitat <- sapply(dataset$Habitat, function(x) {
    if (is.na(x)) return(NA_character_)
    parts <- unlist(strsplit(x, ","))
    parts_unique <- unique(trimws(parts))  # quitamos espacios y duplicados
