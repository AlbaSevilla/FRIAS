MergeDatasets <- function(FileInfo, OutputFiles = TRUE) {
  ...
  #ELIMINAMOS COSAS QUE NO SEAN NUMEROS EN LA COLUMNA DATELIST:
  alldat <- alldat %>%
    mutate(
      DateList = gsub("[^0-9;]", "", DateList)
    )
  #AHORA ORDENAMOS LAS FECHAS DE DATELIST DE MENOR A MAYOR PARA QUE SEA MÁS FACIL
  #COMPROBAR QUE OBTENEMOS LA FECHA MAS ANTIGUA EN OLDESTDATE
  alldat <- alldat %>%
    mutate(
      DateList = sapply(strsplit(DateList, ";"), function(x) {
        x <- str_trim(x)  # quitar espacios en blanco
        x_sorted <- sort(x)  # ordena como texto (si son años funciona bien)
        paste(x_sorted, collapse = ";")
      })
    )
  #Algunas veces sale ;1999 al principio de DateList asi que eliminamos esos comienzos
  alldat <- alldat %>%
    mutate(
      DateList = sub("^;", "", DateList)
    )
  #OLDEST DATE
  alldat <- alldat %>%
    mutate(
  ...
  ORDEN_columnas <- c("OriginalNameDB","AcceptedNameGBIF","ID_GBIF",
                      "Kingdom","Order","Family","FunctionalGroup","Group",
                      "establishmentMeans","pathway",
                      "NativeRangeofIAS_list","invaded_country_list","ISO3_invaded_country_list",
                      "AffectedNativeSpecies","EICATImpact","Mechanism","EICAT_by_Mechanism",
                      "DateList","OldestDate",
                      "habitat","Source_Data")
  ...
  if (nrow(Step2_MasterList_conNA) > 0) {
    write.xlsx(Step2_MasterList_conNA, file.path("OutputFiles", "Final", "NA_AcceptedNameGBIF_ID_GBIF_Masterlist.xlsx"))
    write.csv(Step2_MasterList_conNA, file.path("OutputFiles", "Final", "NA_AcceptedNameGBIF_ID_GBIF_Masterlist.csv"), row.names = FALSE)
  }
}
