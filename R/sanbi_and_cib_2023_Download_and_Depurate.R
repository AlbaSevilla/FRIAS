sanbi_and_cib_2023_Download_and_Depurate <- function() {
  #Download
  url <- "https://zenodo.org/records/14937470/files/A_checklist_of_alien_taxa_for_South%20Africa_v20250520.xlsx?download=1"
  destfile <- "InputFiles/originaldatabase_sanbi_and_cib_2023.xlsx"
  download.file(url, destfile, mode = "wb")

  #Depurate
  dat <- read_excel("InputFiles/originaldatabase_sanbi_and_cib_2023.xlsx")
  sanbi_and_cib_2023_freshwater <- dat %>%
   ...
   sanbi_and_cib_2023_noduplicates <- noduplicates(sanbi_and_cib_2023_freshwater, "scientificName")
   nrow(sanbi_and_cib_2023_noduplicates)
   dataset <- sanbi_and_cib_2023_freshwater
   names <- dataset$scientificName
   names_acep <- character(length(names))
   for (i in seq_along(names)) {
     names_acep[i] <- name_backbone_checklist(names[i])$species
     Sys.sleep(0.02)
   }
  ...
  #Save
  write.xlsx(dataset, "InputFiles/freshwatersubset_sanbi_and_cib_2023.xlsx")
}
