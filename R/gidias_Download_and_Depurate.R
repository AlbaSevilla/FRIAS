gidias_Download_and_Depurate <- function() {
  ...
  names <- gidias_final$IAS_Species_Name
  gidias_final$AcceptedNameGBIF <- name_backbone_checklist(names)$canonicalName
  names(gidias_final)
  gidias_final <- gidias_final %>%
    separate_rows(mechanism_Nature_clean, sep=",") %>%
    mutate(mechanism_Nature_clean = str_remove(mechanism_Nature_clean, "\\s*\\([^)]*\\)")) %>%
    mutate(mechanism_Nature_clean = trimws(mechanism_Nature_clean))
  gidias_final <- noduplicates(gidias_final, "AcceptedNameGBIF")
  #Save
