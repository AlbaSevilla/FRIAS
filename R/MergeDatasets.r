MergeDatasets <- function(FileInfo, OutputFiles = TRUE) {
  ...
  alldat <- alldat %>%
    mutate(
      ReportYear = gsub("[^0-9;]", "", ReportYear),
      ReportYear = sapply(strsplit(ReportYear, ";"), function(x) {
        x <- str_trim(x)
        paste(sort(x), collapse = ";")
      }),
      ReportYear = sub("^;", "", ReportYear),
      EarliestReport = sapply(strsplit(ReportYear, ";"), function(x) {
        x <- str_trim(x)
  ...
  masterlist$CorrectedAcceptedNameGBIF <- masterlist$AcceptedNameGBIF
