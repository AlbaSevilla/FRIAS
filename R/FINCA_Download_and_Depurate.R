FINCA_Download_and_Depurate <- function(){
  ######################################################
  ######### PARA DESCARGAR #############################
  ######################################################
  ...
  destfile <- "InputFiles/Step0_OriginalDatabase_FINCA.xlsx"
  ...
  ########### DEPURAR ##########################
  ##############################################
  dat <- read_excel("Inputfiles/Step0_OriginalDatabase_FINCA.xlsx", col_names = FALSE)
  dat <- dat[-c(1:8),]
  names(dat) <- dat[1,]
  dat <- dat[-1,]
  dat
  ...
  write.csv2(dat_fresh, "Inputfiles/Step0_OriginalDatabaseFreshwaterNODUPLICATES_FINCA.csv")
  data <- read.csv("Inputfiles/Step0_OriginalDatabaseFreshwaterNODUPLICATES_FINCA.csv", stringsAsFactors = FALSE, sep=";")
  write_xlsx(dat_fresh, "Inputfiles/Step0_OriginalDatabaseFreshwaterNODUPLICATES_FINCA.xlsx")
