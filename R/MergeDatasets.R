MergeDatasets <- function(FileInfo, OutputFiles = TRUE) {
  ...
  for (i in seq_along(db_names)) {
    ...
    dat <- suppressMessages(read_csv2(data_Files[i], show_col_types = FALSE))
    issues <- problems(dat)
    if (nrow(issues) > 0) {
      cat("\nProblems detected in ", db_names[i], ":\n")
      print(issues %>% select(row, col, actual, expected))
    }
    dat <- dat %>%
      mutate(
        Impact = tolower(str_trim(Impact)),
  ...
  #Remove species with no Recipient Range Information
  Step2_masterlist <- Step2_masterlist %>%
    filter(
      !(
        (is.na(RecipientRange) | RecipientRange == "" | RecipientRange == "NA") &
          (is.na(RecipientRangeISO3) | RecipientRangeISO3 == "" | RecipientRangeISO3 == "NA")
      )
    )
  ###################################### NAS ####################################################
