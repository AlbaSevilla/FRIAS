peyton_et_al_2019_Download_and_Depurate <- function() {
  ...
  merged2 <- merged %>%
    mutate(
      ImpactMechanism = apply(
        select(., all_of(impact_cols)),
        1,
        function(x) {
          x <- na.omit(x)
          x <- stringr::str_trim(x)
          x <- x[!stringr::str_detect(x, "^(?i)(x|x\\?|\\?)$")]
          x <- stringr::str_replace_all(x, "[^a-zA-Z\\s]", "")
          x <- stringr::str_trim(x)
          x <- x[x != ""]
          if (length(x) == 0) NA_character_
          else paste(unique(x), collapse = "; ")
        }
      )
    )
