StandardizeLocations <- function() {
  MasterList <- read.xlsx("OutputFiles/Intermediate/step9_additionalNativeRangeSInAS_masterlist.xlsx")
  ###############################
  ####### RECIPIENT RANGE #######
  ###############################
  #Clean cells text
  ...
  get_native_Range <- function(native_str) {
    ...
    for (item in native_items) {
      ...
      # STRICT PARTIAL MATCH (avoids substring contamination)
      keyword_partial <- locations_table$Location[
        str_detect(
          locations_table$keywords,
          regex(paste0("\\b", item, "\\b"))
        )
      ]
    ...
    return(list(
      matched = if (length(matched_locations) == 0) NA_character_
      else paste(matched_locations, collapse = "; "),
      not_matched = if (length(not_matched) == 0) NA_character_
      else paste(not_matched, collapse = "; ")
    ))
  }
  ...
  # 8. Export results
  write_csv(MASTERLIST_noduplicates,"OutputFiles/Intermediate/step10_standardlocationnames_masterlist.csv")
  write_xlsx(MASTERLIST_noduplicates,"OutputFiles/Intermediate/step10_standardlocationnames_masterlist.xlsx")
}
