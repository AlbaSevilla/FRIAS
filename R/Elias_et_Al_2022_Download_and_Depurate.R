Elias_et_Al_2022_Download_and_Depurate <- function(){
  ...
  destfile <- "InputFiles/Step0_OriginalDatabase_Elias_et_Al_2022.xlsx"
  ...
  ########### DEPURAR ##########################
  ##############################################
  dat <- read_excel("Inputfiles/Step0_OriginalDatabase_Elias_et_Al_2022.xlsx", col_names = FALSE)
  dat <- dat[-c(1),]
  colnames(dat) <- dat[1,]
  dat <- dat[-c(1),]
  ...
  write_xlsx(dat_fresh2, "Inputfiles/Step0_OriginalDatabaseFreshwaterNODUPLICATES_Elias_et_Al_2022.xlsx")
}
