...
HarmonizationDatabases <- function (CorrespondenceTable){
  ...
  for (i in 1:nrow(CorrespondenceTable)){
    ...
    equivs <- read_excel(
      ...
      mutate(
        Database = Database,
        OriginalCategories = tolower(trimws(as.character(OriginalCategories))),
        StandardizedCategories = tolower(trimws(as.character(StandardizedCategories))),

        # Quitar .0 SOLO cuando el valor es un número individual
        OriginalCategories = ifelse(
          grepl("^[0-9]+\\.0$", OriginalCategories),
          sub("\\.0$", "", OriginalCategories),
          OriginalCategories
        ),

        StandardizedCategories = ifelse(
          grepl("^[0-9]+\\.0$", StandardizedCategories),
          sub("\\.0$", "", StandardizedCategories),
    ...
    desired_order <- c("AcceptedNameGBIF", "OriginalNameDB", "ID_GBIF",
                       "Kingdom", "Order", "Family", "Phylum","Class",
