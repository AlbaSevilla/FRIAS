NISIC_Download_and_Depurate <- function(){
  ...
  NISIC_names <- nombre_cientifico4 #Nombres científicos de las especies
  ...
  NISIC_allspecies <- dataset_final %>%
    select(-`Common Name`)

  # Simulamos una columna como la tuya (aquí deberías usar NISIC_allspecies$`Native To`)
  native_clean <- NISIC_allspecies$`Native To` %>%
  ...
  NISIC_allspecies$`Native_Countries_Only` <- native_clean

  names(NISIC_allspecies)
  names(NISIC_allspecies) <- gsub(" ", "_", names(NISIC_allspecies))
  names(NISIC_allspecies) <- gsub("\\.", "", names(NISIC_allspecies))

  # Renombrar la columna "Native_Countries_Only" a "Native_Range"
  names(NISIC_allspecies)[names(NISIC_allspecies) == "Native_Countries_Only"] <- "Native_Range"
  ...
  NISIC_allspecies$Date_of_US_Introduction <- sapply(NISIC_allspecies$Date_of_US_Introduction, extract_all_years)


  write.xlsx(NISIC_allspecies, "./Inputfiles/Step0_OriginalDatabase_NISIC.xlsx")
  cat("Archivo descargado correctamente: Step0_OriginalDatabase_NISIC.xlsx", "\n")
  write.csv2(NISIC_allspecies, "./Inputfiles/Step0_OriginalDatabase_NISIC.csv")
  cat("Archivo descargado correctamente: Step0_OriginalDatabase_NISIC.csv", "\n")
  ...
  write.xlsx(dataset_freshwater, "./Inputfiles/Step0_OriginalDatabaseFreshwaterNODUPLICATES_NISIC.xlsx")
