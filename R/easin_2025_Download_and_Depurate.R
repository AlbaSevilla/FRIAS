easin_2025_Download_and_Depurate <- function(){
  ...
  names <- final_dataset$Name
  final_dataset$AcceptedNameGBIF <- name_backbone_checklist(names)$canonicalName
  final_dataset <- final_dataset %>%
    mutate(
      Invaded_Countries = if_else(
        is.na(Invaded_Countries) | Invaded_Countries == "" | Invaded_Countries == "NA",
        "Europe",
        Invaded_Countries
      )
    )
