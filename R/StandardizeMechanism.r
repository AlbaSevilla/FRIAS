StandardizeMechanism <- function() {
  ...
  dat$Mechanisms_std[is.na(replacements)] <- NA

  source(file.path("R", "noduplicates.r"))
  MasterlistStandardized <- noduplicates(dat, "AcceptedNameGBIF")

  write_xlsx(MasterlistStandardized,
             "OutputFiles/Intermediate/step10_standardizedmechanism_masterlist.xlsx")

  write_csv(MasterlistStandardized,
            "OutputFiles/Intermediate/step10_standardizedmechanism_masterlist.csv")
}
