SalmonidosChile_Download_and_Depurate <- function(){
  ###########################################################
  ################## DESCARGAR ##############################
  ###########################################################
  url <- "https://www.revistaecosistemas.net/index.php/ecosistemas/article/download/1008/874/3237"
  destfile <- "InputFiles/Step0_OriginalDatabase_SalmonidosChile.pdf"
  ...
  df <- as.data.frame(datos_raw, stringsAsFactors = FALSE)

  # Rellenar 'orden' y 'familia' vacíos con el valor anterior
  for (col in c("orden", "familia")) {
    for (i in 2:nrow(df)) {
      if (df[i, col] == "") df[i, col] <- df[i - 1, col]
    }
  }
  ...
  df <- df[,columnas_seleccion]
  rownames(df) <- NULL
  str(df)
  df$rango_nativo <- sub(".*?\\)", "", df$rango_nativo)
  df$especie <- sub("\\(.*", "", df$especie)

  write.xlsx(df, "Inputfiles/Step0_OriginalDatabase_SalmonidosChile.xlsx")
  ...
  write.xlsx(freshwater_species, "Inputfiles/Step0_OriginalDatabaseFreshwaterNODUPLICATES_SalmonidosChile.xlsx")
}
