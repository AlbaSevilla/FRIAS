INVASIBER_Download_and_Depurate <- function(){
  ###################################################################
  ####### INVASIBER "otros" #########################################
  ###################################################################
  ...
  dataset_invasiber <- as.data.frame(df_unido)
  dataset_invasiber2 <- dataset_invasiber[,-c(9:12)]

  names(dataset_invasiber2)
  names(dataset_invasiber2)[names(dataset_invasiber2) == "Especie:"] <- "Especie"
  names(dataset_invasiber2)[names(dataset_invasiber2) == "Descripción:"] <- "Descripcion"
  names(dataset_invasiber2)[names(dataset_invasiber2) == "Biología\n                      y hábitat:"] <- "BiologiayHabitat"
  names(dataset_invasiber2)[names(dataset_invasiber2) == "Distribución\n                      geográfica nativa:"] <- "DistribucionGeograficaNativa"
  colnames(dataset_invasiber2)[5] <- "Distribucion"
  names(dataset_invasiber2)[names(dataset_invasiber2) == "Mecanismo\n                      de introducción:"] <- "MecanismoIntroduccion"
  names(dataset_invasiber2)[names(dataset_invasiber2) == "Impacto\n                      ecológico:"] <- "ImpactoEcologico"
  names(dataset_invasiber2)[names(dataset_invasiber2) == "Impacto\n                      socioeconómico:"] <- "ImpactoSocioeconomico"

  write_xlsx(dataset_invasiber2,"Inputfiles/Step0_OriginalDatabase_INVASIBER.xlsx")
  cat("Archivo guardado correctamente : Inputfiles/Step0_OriginalDatabase_INVASIBER.xlsx")

  ############################################
  ###### ESPECIES FRESHWATER #################
  ############################################
  source(file.path("R", "check_habitat.r"))
  dataset <- dataset_invasiber2
  nombre_especies <- dataset_invasiber2$Especie
  ...
  write_xlsx(dataset_freshwater2, "Inputfiles/Step0_OriginalDatabaseFreshwaterNODUPLICATES_INVASIBER.xlsx")
  cat("Archivo guardado correctamente : Inputfiles/Step0_OriginalDatabaseFreshwaterNODUPLICATES_INVASIBER.xlsx")
}
