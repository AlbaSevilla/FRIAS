oliva_paterna_et_al_2022_life_invasaqua_Download_and_Depurate <- function() {
  #Depurate
  data_blacklist <- read.xlsx(file.path("InputFiles","originaldatabase_oliva_paterna_et_al_2022_life_invasaqua.xlsx"),sheet=3)
  data_alertlist <- read.xlsx(file.path("InputFiles","originaldatabase_oliva_paterna_et_al_2022_life_invasaqua.xlsx"),sheet=4)
 ...
 data_subset_blacklist3 <- data_subset_blacklist2 %>%
  ...
  oliva_paterna_dataset <- rbind(dataset_freshwater_blacklist, dataset_freshwater_alertList)
  oliva_paterna_dataset$RecipientRange <- "Spain; Portugal"

  #Save
  write.xlsx(oliva_paterna_dataset, "./InputFiles/freshwatersubset_oliva_paterna_et_al_2022_life_invasaqua.xlsx")
}
